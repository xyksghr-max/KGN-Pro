...
class GraspPoseDataset(data.Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        _, _, obj_types, _, _, grasp_poses_gt, grasp_widths_gt, _ = self._get_scene_info(scene_idx)

        # 2.28
        TARGET_SINGLE_POSE_SHAPE = (4, 4)
        TARGET_NUM_POSES_PER_SAMPLE = 80
        if(len(grasp_poses_gt) == 1):
            TARGET_BATCH_SIZE = 1
        else:
            TARGET_BATCH_SIZE = 6 
        processed_sample_grasps = []

        for i, sample_grasps in enumerate(grasp_poses_gt):
            current_sample_tensor = None

            if isinstance(sample_grasps, np.ndarray):
                current_sample_tensor = torch.tensor(sample_grasps, dtype=torch.float32)
            elif torch.is_tensor(sample_grasps):
                current_sample_tensor = sample_grasps.to(torch.float32)
            else:
                current_sample_tensor = torch.empty(0, *TARGET_SINGLE_POSE_SHAPE, dtype=torch.float32)

            if not (current_sample_tensor.ndim == 3 and current_sample_tensor.shape[1:] == TARGET_SINGLE_POSE_SHAPE):
                current_sample_tensor = torch.empty(0, *TARGET_SINGLE_POSE_SHAPE, dtype=torch.float32)

            current_num_poses_in_sample = current_sample_tensor.shape[0] 

            unified_sample_tensor = None
            if current_num_poses_in_sample == TARGET_NUM_POSES_PER_SAMPLE:
                unified_sample_tensor = current_sample_tensor
            elif current_num_poses_in_sample < TARGET_NUM_POSES_PER_SAMPLE:
                if current_num_poses_in_sample == 0:
                    unified_sample_tensor = torch.zeros(
                        (TARGET_NUM_POSES_PER_SAMPLE, *TARGET_SINGLE_POSE_SHAPE),
                        dtype=torch.float32
                    )
                else:
                    repeat_factor = (TARGET_NUM_POSES_PER_SAMPLE + current_num_poses_in_sample - 1) // current_num_poses_in_sample
                    expanded_tensor = current_sample_tensor.repeat(repeat_factor, 1, 1)
                    unified_sample_tensor = expanded_tensor[:TARGET_NUM_POSES_PER_SAMPLE, :, :]
            else: # current_num_poses_in_sample > TARGET_NUM_POSES_PER_SAMPLE
                unified_sample_tensor = current_sample_tensor[:TARGET_NUM_POSES_PER_SAMPLE, :, :]

            processed_sample_grasps.append(unified_sample_tensor)


        final_gt_grasp_batch = None
        if processed_sample_grasps:
            combined_batch_tensor = torch.stack(processed_sample_grasps, dim=0)
            current_batch_size = combined_batch_tensor.shape[0]

            if current_batch_size == TARGET_BATCH_SIZE:
                final_gt_grasp_batch = combined_batch_tensor
            elif current_batch_size < TARGET_BATCH_SIZE:
                dummy_sample = torch.zeros(
                    (1, TARGET_NUM_POSES_PER_SAMPLE, *TARGET_SINGLE_POSE_SHAPE),
                    dtype=torch.float32
                )
                num_to_pad = TARGET_BATCH_SIZE - current_batch_size
                padding_tensor = dummy_sample.repeat(num_to_pad, 1, 1, 1)
                final_gt_grasp_batch = torch.cat([combined_batch_tensor, padding_tensor], dim=0)
            else: # current_batch_size > TARGET_BATCH_SIZE
                final_gt_grasp_batch = combined_batch_tensor[:TARGET_BATCH_SIZE, :, :]
        else:
            final_gt_grasp_batch = torch.zeros(
                (TARGET_BATCH_SIZE, TARGET_NUM_POSES_PER_SAMPLE, *TARGET_SINGLE_POSE_SHAPE),
                dtype=torch.float32
            )


        gt_shape = len(grasp_poses_gt[0])
        scene = scene_idx
       
        ret.update({'grasp_pose':final_gt_grasp_batch})
        ret.update({'gt_shape':gt_shape})
