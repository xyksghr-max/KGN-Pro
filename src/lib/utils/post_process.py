...
import numpy as np
import torch
from .image import transform_preds,transform_preds_pro
...
def grasp_pose_post_process_pro(opt, dets, c, s, h, w):
    dets.requires_grad_(True)
    ret = []
    for i in range(dets.size(0)):
        det_pro = dets[i]
        centers = transform_preds_pro(det_pro[:, :2].view(-1, 2), c[i], s[i], (w, h))
        kpts = transform_preds_pro(det_pro[:, 2:10].reshape(-1, 2), c[i], s[i], (w, h))
        top_preds = torch.cat([
            centers.view(-1, 2), 
            kpts.view(-1, 8), 
            det_pro[:, 10:]
        ], dim=1)
        top_preds.requires_grad_(True)
        ret.append({torch.tensor([0], dtype=torch.int32): top_preds})
    ret_tensor = [item[next(iter(item))] for item in ret]  
    ret_tensor = torch.stack(ret_tensor) 
    return ret_tensor
