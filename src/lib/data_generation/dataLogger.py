...
class DataLogger():
    ...
    #         dep_image_normalized = cv2.normalize(dep_image, None, 0, 255, cv2.NORM_MINMAX)
    #         dep_image_uint8 = dep_image_normalized.astype(np.uint8)

    #         # Create custom colormap (green to blue without bright red)
    #         custom_colormap = np.zeros((256, 1, 3), dtype=np.uint8)
    #         for j in range(256):
    #             if j < 220:
    #                 # Low values: green (small depth)
    #                 custom_colormap[j, 0] = [0, j*2 , 0]  # green
    #             else:
    #                 # High values: blue (large depth)
    #                 blue_value = 255 - (j - 128) * 2
    #                 custom_colormap[j, 0] = [blue_value//2 , 0, blue_value]  # soft blue

    #         # Normalize the custom colormap
    #         custom_colormap = custom_colormap[:, 0].reshape(256, 1, 3)

    #         # Apply the custom colormap
    #         dep_image_color = cv2.applyColorMap(dep_image_uint8, custom_colormap)
    ...
    def save_depths(self, depths):
        ...
        for i in range(N_cam):
            ...
            # Apply the 'inferno' colormap for better visualization
            # Normalize depth values to the range [0, 1] for colormap application
            dep_normalized = np.clip(dep / np.max(dep), 0, 1)

            # Use Matplotlib's inferno colormap
            dep_colored = plt.cm.inferno(dep_normalized)[:, :, :3]  # Get RGB channels
            
            # Convert the colormap result to a format suitable for saving with OpenCV
            dep_colored = (dep_colored * 255).astype(np.uint8)  # Scale to 0-255
            
            # Save the depth image with inferno colormap
            cv2.imwrite(
                os.path.join(self.depth_img_dir, 'depth_image_{}.png'.format(i)), 
                dep_colored
            )
