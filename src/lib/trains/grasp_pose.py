...
from utils.post_process import grasp_pose_post_process_pro
...
class GraspPoseLoss_clf(torch.nn.Module):
    ...
    def forward(self, outputs, batch):
        ...
        for s in range(opt.num_stacks):
            ...
            # print(batch['reg'].size())
            # print(output['reg'].size())
            ...
            # batch['w'] [batch_size, max_grasps, 9]
            # output['w'] [batch_size, 9, output_res, output_res]
            # w_mask [batch_size, max_grasps, 9]
            if opt.w_weight > 0:
        ...
        loss_stats = {'loss': loss, 'hm_loss': hm_loss, 'w_loss': w_loss,
                    ...
                    "scale_loss": scale_loss, 'loss_mc_all':loss_mc_all
                    }
...
class GraspPoseTrainer(BaseTrainer):
    ...
    def _get_losses(self, opt):
        loss_states = ['loss', 'hm_loss', 'w_loss', 'kpts_center_loss',
                   'reg_loss(center_offset)','loss_mc_all',]
    ...
    def post_process(self, dets, meta, scale=1):   
        #dets = dets.detach().cpu().numpy().reshape(1, -1, dets.shape[2])
        #assert np.array_equal(meta['c'], np.array([320.,240.])), "meta is not equal"
        assert torch.equal(meta['c'], torch.tensor([320.,240.], dtype=torch.float32, device=dets.device)), "meta is not equal"
        dets = grasp_pose_post_process_pro(
            self.opt,
            dets.clone().to(dets.device), 
            [meta['c'].clone().to(dets.device)], 
            [meta['s'].clone().to(dets.device)],  
            torch.tensor(meta['out_height'], dtype=torch.int32).to(dets.device), 
            torch.tensor(meta['out_width'], dtype=torch.int32).to(dets.device)
        )

        for j in range(0, self.opt.num_classes ):
            if self.opt.sep_scale_branch:
                dets[j] = dets[j].float().view(-1, 2 + 2*4 + 1 + 1 + 1 + 1)
            else:
                dets[j] = dets[j].float().view(-1, 2 + 2*4 + 1 + 1 + 1)
            # grasp_pose_post_process_
            dets[j][:,:10] =  dets[j][:,:10] / scale
        return dets[0]
