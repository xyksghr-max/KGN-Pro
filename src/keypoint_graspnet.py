...
class KeypointGraspNet():
    ...
    def generate(self, input, return_all=False):
        ...
        if not self.opt.test_oracle_kpts:
            # num
            # kpts.shape = (num,4,2)  
            # centers.shape = (num,1,2)
            # widths_gen = num
            # scales = none
            # scores.shape = 100..
            kpts, centers, widths_gen, scales, scores, ouptput = self._generate_kpts(detector_input)
        else:
        ...
        if not return_all:
            return np.array(quaternions), np.array(locations), np.array(widths), np.stack(kpts_keep) if len(kpts_keep) > 0 else np.array([]), np.array(scores_keep), ouptput
        else:
    ...
    def _generate_kpts(self, input):
        ...
        # input = oracle_dets_or_pre_processed_images
        ...
        if self.opt.sep_scale_branch:
            ...
            return kpts_2d_pred, centers_2d_pred, widths_pred, scales, scores, ret['output']
        else:
            return kpts_2d_pred, centers_2d_pred, widths_pred, None, scores, ret['output']
