...
def train(epoch, cfg, data_loader, model, obj_info, criterions, optimizer=None):
    ...
    for i, (obj, obj_id, inp, target, loss_msk, trans_local, pose, c_box, s_box, box) in enumerate(data_loader):
        ...
        if cfg.pytorch.gpu > -1:
            ...
            c_box_var = c_box.cuda(cfg.pytorch.gpu).float() # [[342., 278.]]
            s_box_var = s_box.cuda(cfg.pytorch.gpu).float() # [124.]
        else:
            inp_var = inp.float()
        ...
        bs = len(inp)       # batch size
        # forward propagation
        T_begin = time.time()
        # import ipdb; ipdb.set_trace()
        noc, w2d, scale = model(inp_var)  # CDPN  
        """
        noc(bs,3,64,64) 
        w2d(bs,2,64,64) 
        scale (bs,2) 
        ...
        if 'rot' in cfg.pytorch.task.lower() and not cfg.network.rot_head_freeze:
            ...
            s = s_box_var.to(torch.int64)       # scale (bs, )
            ...
            rot_mat = pose_var[:, :, :3]  # (bs, 3, 3)  BatchSize * R
            trans_vec = pose_var[:, :, 3]  # (bs, 3)    BatchSize * t
            rot_quat = matrix_to_quaternion(rot_mat)    # R
            pose_gt = torch.cat((trans_vec, rot_quat), dim=-1)  # BS * 7 
            ...
            x3d = x3d.flatten(2).transpose(-1, -2)[batch_inds, sample_inds]     # (bs,sample_inds,3)
            x2d = x2d.flatten(2).transpose(-1, -2)[batch_inds, sample_inds]     # (bs,sample_inds,2)
            w2d = w2d.flatten(2).transpose(-1, -2)[batch_inds, sample_inds]     # (bs,sample_inds,2)
            w2d = w2d.softmax(dim=1) * (scale[:, None, :] / wh_unit[:, None, None])

            allowed_border = 30 * wh_unit  # (n, )
            camera = PerspectiveCamera(
                cam_mats=cam_intrinsic[None].expand(bs, -1, -1),    # camera
                z_min=0.01,
