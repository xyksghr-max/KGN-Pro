...
import sys 
sys.path.append('../EPro-PnP-6DoF_v2/lib/ops/utils')
from img import zoom_in, get_edges, xyxy_to_xywh, Crop_by_Pad
from transform3d import prj_vtx_cam
from ioio import read_pickle
import fancy_logger as logger
from eval import calc_rt_dist_m 
...
class LM(data.Dataset):
    ...
    def change_bg(self, rgb, msk):
        ...
        msk = np.dstack([msk, msk, msk]).astype(bool)
        bg_im[msk] = rgb[msk]
        return bg_im
    ...
    def __getitem__(self, idx):
        if self.split == 'train':
            obj = self.load_obj(idx)        # ape、camera、cat...
            obj_id = ref.obj2idx(obj)       # ape:1  camera:2
            data_type = self.load_type(idx) # real
            box = self.load_box(idx)        # box.txt 
            pose = self.load_pose(idx)      # pose.txt，R+t
            rgb = self.load_rgb(idx)        # color.png
            if data_type == 'real':
            ...
            if (self.split == 'train') and self.cfg.dataiter.dzi:  # pad_tatio:1.5
