...
class LMSolver(nn.Module):
    ...
    def forward(self, x3d, x2d, w2d, camera, cost_fun, with_pose_opt_plus=False,
        ...
        if isinstance(normalize_override, bool):
            normalize = normalize_override  # False
        else:
    ...
    def _lm_iter(
        ...
        new_residual = residual_new.clone()
        new_cost = cost_new.clone()
        new_jac[~step_is_successful] = jac[~step_is_successful]
        new_residual[~step_is_successful] = residual[~step_is_successful]
        new_cost[~step_is_successful] = cost[~step_is_successful]
        jac = new_jac
        ...
        diagonal += (diagonal.clamp(min=self.min_lm_diagonal, max=self.max_lm_diagonal
                            ) / radius[:, None] + self.eps).clamp(min=self.eps)
...
class RSLMSolver(LMSolver):
    ...
    def center_based_init(self, x2d, x3d, camera, eps=1e-6):
        ...
        x2dc_std, x2dc_mean = torch.std_mean(x2dc, dim=-2)      # (bs,2,2) 
    ...
    def solve(self, x3d, x2d, w2d, camera, cost_fun, **kwargs):
        with torch.no_grad():
            ...
            if bs > 0:
                ...
                # (num_obj, num_pts, 2) -> (1, num_obj, num_pts) -> (num_proposals, num_obj, num_pts)
                inds = torch.multinomial(
                    mean_weight.reshape(-1, pn), self.num_points     
                ).reshape(self.num_proposals, bs, self.num_points)   
                # (num_proposals, num_obj, num_pts) -> (num_proposals, num_obj, num_points)
