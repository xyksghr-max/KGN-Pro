...
class EProPnP6DoF(EProPnPBase):
    ...
    def initial_fit(self,
        ...
        #            + eye_4).inverse()
        regularized_cov = pose_cov[:, 3:, 3:]  # 
        pseudo_inverse_cov = torch.pinverse(regularized_cov)
        rot_cov = (transform_mat @ pseudo_inverse_cov @ transform_mat.transpose(-1, -2)
                + eye_4).inverse()
    ...
    def gen_new_distr(iter_id, trans_mode, trans_cov_tril, rot_cov_tril):
        ...
        trans_mode_iter = trans_mode[iter_id]
        trans_cov_tril_iter = trans_cov_tril[iter_id]
        if torch.isnan(trans_mode_iter).any():
            print(f"NaN detected in trans_mode at iteration {iter_id}")
            trans_mode_iter = torch.nan_to_num(trans_mode_iter) 
            trans_mode_iter = torch.clamp(trans_mode_iter, min=-1e4, max=1e4)
        if torch.isnan(trans_cov_tril_iter).any():
            print(f"NaN detected in trans_cov_tril at iteration {iter_id}")
            trans_cov_tril_iter = torch.nan_to_num(trans_cov_tril_iter)  
            trans_cov_tril_iter = torch.clamp(trans_cov_tril_iter, min=-1e4, max=1e4)
        new_trans_distr = MultivariateStudentT(3, trans_mode_iter, trans_cov_tril_iter)
